...
export default function DirectoryPage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6">
          {/* Example fields. Adjust or remove as needed */}
          {startup.problem && (
          ...
          {/* Display founders only if display_founders = true */}
          {startup.display_founders && startup.founders && (
            <div className="space-y-2">
              <h3 className="font-semibold text-gray-900">Founders</h3>
              <p className="text-gray-700">{startup.founders}</p>
            </div>
          )}
        </div>
    ...
    return (
      <div className="flex justify-center items-center space-x-2 mt-12">
        <button
          ...
          className="px-3 py-2 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-blue-50 hover:text-blue-600 disabled:opacity-50 transition-colors"
        >
          Previous
        </button>
  ...
  // ----- Loading Startups from Supabase -----
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const filtered = startups.filter((startup) => {
      // Search match: check name + description
      const matchesSearch =
        (startup.name?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
        (startup.description?.toLowerCase() || '').includes(
          searchTerm.toLowerCase()
        );

      // Filter match by industry / stage / team
      const matchesFilter =
        selectedFilter === 'All' ||
        (filterType === 'industry'
          ? startup.industry
        ...
        <div className="max-w-6xl mx-auto space-y-8 relative">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16 relative">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Discover and connect with innovative startups from the Yale ecosystem
            </p>
          </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center bg-white border border-gray-200 rounded-lg px-3 py-3">
                  ...
                  <select
                    ...
                    value={filterType}
                    onChange={(e) =>
                      setFilterType(e.target.value as keyof FilterOptions)
                    }
                  >
          ...
          <div className="flex justify-between items-center mb-6">
            <p className="text-sm text-gray-600">
              Showing {(currentPage - 1) * startupsPerPage + 1} -{' '}
              {Math.min(currentPage * startupsPerPage, filteredStartups.length)}{' '}
              of {filteredStartups.length} startups
          ...
          <div className="mt-24 mb-12">
            <h2 className="text-2xl font-semibold text-blue-900 mb-6 text-center">
              Additional Yale Startup Resources
            </h2>
            <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <div className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 p-5 h-full flex flex-col">
                  ...
                  <p className="text-sm text-gray-600 mb-4 flex-grow">
                    Explore other student-led ventures and innovative projects
                    supported by Yale's Innovation Centers.
                  </p>
              ...
              >
                <div className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all duration-200 p-5 h-full flex flex-col">
                  ...
                  <p className="text-sm text-gray-600 mb-4 flex-grow">
                    Discover technology spinouts and research-driven ventures
                    emerging from Yale's labs and research centers.
                  </p>
                  <div className="inline-flex items-center text-sm text-blue-600 font-medium group-hover:text-blue-700">
